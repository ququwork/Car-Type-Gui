...
#c:\users\tlkj\appdata\local\programs\python\python37\python.exepip
import torch
import torchvision
import paramiko
import cv2
...
class Car_DC_NET():
    ...
    def get_url_pic(self, url: str, savePath: str, carNumber: str):
        #TODO 二期图片配置 网络图片流获取
        urlPic = BytesIO(requests.get(url).content)

        # 截取号牌所识别车辆
        image = Image.open(urlPic)
        image_new = Image.new("RGB", image.size, "#FFFFFF")
        carNumbers = carNumber.split('-')
        print("carNumbers:", carNumbers)
        carBox = [int(carNumbers[0])-1000, int(carNumbers[1])-800, int(carNumbers[2])+600, int(carNumbers[3])+600]
        # carBox = [int(carNumbers[0]), int(carNumbers[1]), int(carNumbers[2]), int(carNumbers[3])]
        ...
        if carBox[1] < 0:
            carBox[1] = 0
        if carBox[2] > image.size[0]:
            carBox[2] = image.size[0]
        if carBox[3] > image.size[1]:
            carBox[3] = image.size[1]
        print("carBox2: ", carBox)
        print("url: ", url)

        carBox = tuple(carBox)
        image_car = image.crop(carBox)
        image_new.paste(image_car, (10, 10))
        image_new.save(savePath)
    ...
    def detect_classify(self):
        ...
        for x in self.imgs_path:
            ...
            orig_img = cv2.cvtColor(np.asarray(
                img), cv2.COLOR_RGB2BGR)  # RGB => BGR
            if type(output) != int:
                CAR_TYPE = self.cls_draw_bbox(output, orig_img)   # 识别
                dst_path = self.dst_dir + '/' + os.path.split(x)[1]
                if not os.path.exists(dst_path):
                    cv2.imwrite(dst_path, orig_img)
                return CAR_TYPE
...
if __name__ == '__main__':
    ...
    url2 = 'http://10.118.84.58:8090/Pics1//illepict/2019/07/12/16/311500000000099007000000000001/0120190712163415-683-1.jpg'
    savePath = './test_imgs' + '/pic.jpg'

    DR_model = Car_DC_NET(src_dir='./test_imgs', dst_dir='./test_result', pic_url=url)
    DR_model.get_url_pic(url2, savePath)   # 接口 提供网络图片
